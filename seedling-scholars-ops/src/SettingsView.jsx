...
function RegulationsPanel() {
  ...
  async function toggleActive(r) {
    await supabase.from('compliance_requirements').update({ active: !r.active }).eq('id', r.id);
    loadStateData(selectedState);
  }
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          <div className="surface" style={{ padding: 20, maxWidth: 780 }}>
            ...
            <div className="divide-token">
              ...
              {docs.map((d) => (
                <div key={d.id} style={{ padding: '10px 0', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1 }}>
          ...
          <div className="surface" style={{ padding: 20, maxWidth: 780 }}>
            ...
            <div className="divide-token">
              {reqs.length === 0 && <div style={{ padding: '10px 0', color: 'var(--ink-faint)', fontSize: 13 }}>No requirements yet.</div>}
              {reqs.map((r) => (
                <div key={r.id} style={{ padding: '11px 0', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: 160 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: r.active ? 'var(--ink)' : 'var(--ink-faint)' }}>{r.label}</div>
                  ...
                  <span className={`chip ${r.active ? 'chip-good' : 'chip-neutral'}`} style={{ cursor: 'pointer' }} onClick={() => toggleActive(r)}>
                    {r.active ? 'Active' : 'Off'}
                  </span>
