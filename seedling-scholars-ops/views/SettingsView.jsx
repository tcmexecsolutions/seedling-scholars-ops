...
function StaffPanel() {
  ...
  async function createStaff() {
    ...
    if ((draft.role === 'director' || draft.role === 'teacher') && !draft.site_id) {
      setError('Directors and teachers need a house assigned.');
      return;
    }
    if (draft.role === 'account_holder' && draft.site_ids.length === 0) {
      setError('Account holders need at least one house assigned.');
      return;
    }
    setSaving(true);
    const { data, error: fnError } = await supabase.functions.invoke('admin-create-staff', {
      body: {
        ...
        role: draft.role,
        site_id: draft.role === 'director' || draft.role === 'teacher' ? draft.site_id : null,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div>
                  ...
                  <select className="field" style={{ marginTop: 4 }} value={draft.role} onChange={(e) => setDraft((d) => ({ ...d, role: e.target.value }))}>
                    ...
                    <option value="director">Site Director — one house</option>
                    <option value="account_holder">Account Holder — one or more houses</option>
                ...
                {(draft.role === 'director' || draft.role === 'teacher') && (
                  <div>
                ...
                {draft.role === 'account_holder' && (
                  <div>
                    <label style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--ink-soft)' }}>Houses this account holder covers</label>
